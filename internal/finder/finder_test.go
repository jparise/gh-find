...
func TestShouldIncludeRepo(t *testing.T) {
	tests := []struct {
		...
		// Source repositories
		{
			name: "source repo with sources filter",
			repo: &github.Repository{
				Fork:      false,
				Archived:  false,
				MirrorURL: "",
		...
		{
			name: "source repo without sources filter",
			repo: &github.Repository{
				Fork:      false,
				Archived:  false,
				MirrorURL: "",
		...
		{
			name: "fork without forks filter",
			repo: &github.Repository{
				Fork:      true,
				Archived:  false,
				MirrorURL: "",
		...
		// Archived repositories (independent attribute)
		{
			name: "archived source with archives filter",
			repo: &github.Repository{
				Fork:      false,
				Archived:  true,
				MirrorURL: "",
		...
		{
			name: "archived source without archives filter",
			repo: &github.Repository{
				Fork:      false,
				Archived:  true,
				MirrorURL: "",
		...
		{
			name: "archived fork with both filters",
			repo: &github.Repository{
				Fork:      true,
				Archived:  true,
				MirrorURL: "",
		...
		{
			name: "archived fork with fork filter only",
			repo: &github.Repository{
				Fork:      true,
				Archived:  true,
				MirrorURL: "",
		...
		{
			name: "archived fork with archives filter only",
			repo: &github.Repository{
				Fork:      true,
				Archived:  true,
				MirrorURL: "",
		...
		// Mirror repositories
		{
			name: "mirror with mirrors filter",
			repo: &github.Repository{
				Fork:      false,
				Archived:  false,
				MirrorURL: "https://example.com/repo.git",
			},
			filter: &RepoFilter{
				IncludeSources:  false,
				IncludeForks:    false,
		...
		{
			name: "mirror without mirrors filter",
			repo: &github.Repository{
				Fork:      false,
				Archived:  false,
				MirrorURL: "https://example.com/repo.git",
		...
		// All filter
		{
			name: "archived fork with all filter",
			repo: &github.Repository{
				Fork:      true,
				Archived:  true,
				MirrorURL: "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := shouldIncludeRepo(tt.repo, tt.filter)
			if got != tt.want {
...
func TestParseRepoTypes(t *testing.T) {
	tests := []struct {
		name      string
		repoTypes []github.RepoType
		want      *RepoFilter
	}{
		{
			name:      "sources only",
			repoTypes: []github.RepoType{github.RepoTypeSources},
			want: &RepoFilter{
				IncludeSources:  true,
				IncludeForks:    false,
				IncludeArchived: false,
		...
		{
			name:      "forks only",
			repoTypes: []github.RepoType{github.RepoTypeForks},
			want: &RepoFilter{
				IncludeSources:  false,
				IncludeForks:    true,
				IncludeArchived: false,
		...
		{
			name:      "archives only",
			repoTypes: []github.RepoType{github.RepoTypeArchives},
			want: &RepoFilter{
				IncludeSources:  false,
				IncludeForks:    false,
				IncludeArchived: true,
		...
		{
			name:      "mirrors only",
			repoTypes: []github.RepoType{github.RepoTypeMirrors},
			want: &RepoFilter{
				IncludeSources:  false,
				IncludeForks:    false,
				IncludeArchived: false,
		...
		{
			name:      "all types",
			repoTypes: []github.RepoType{github.RepoTypeAll},
			want: &RepoFilter{
				IncludeSources:  true,
				IncludeForks:    true,
				IncludeArchived: true,
		...
		{
			name:      "sources and forks",
			repoTypes: []github.RepoType{github.RepoTypeSources, github.RepoTypeForks},
			want: &RepoFilter{
				IncludeSources:  true,
				IncludeForks:    true,
				IncludeArchived: false,
		...
		{
			name:      "forks and archives",
			repoTypes: []github.RepoType{github.RepoTypeForks, github.RepoTypeArchives},
			want: &RepoFilter{
				IncludeSources:  false,
				IncludeForks:    true,
				IncludeArchived: true,
		...
		{
			name:      "sources, forks, and archives",
			repoTypes: []github.RepoType{github.RepoTypeSources, github.RepoTypeForks, github.RepoTypeArchives},
			want: &RepoFilter{
				IncludeSources:  true,
				IncludeForks:    true,
				IncludeArchived: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := parseRepoTypes(tt.repoTypes)
			if got.IncludeSources != tt.want.IncludeSources ||
				got.IncludeForks != tt.want.IncludeForks ||
				got.IncludeArchived != tt.want.IncludeArchived ||
				got.IncludeMirrored != tt.want.IncludeMirrored {
				t.Errorf("parseRepoTypes() = %+v, want %+v", got, tt.want)
			}
		})
	}
}
...
func TestFilterRepos(t *testing.T) {
	repos := []*github.Repository{
		{FullName: "user/source", Fork: false, Archived: false, MirrorURL: ""},
		{FullName: "user/fork", Fork: true, Archived: false, MirrorURL: ""},
		{FullName: "user/archived", Fork: false, Archived: true, MirrorURL: ""},
		{FullName: "user/archived-fork", Fork: true, Archived: true, MirrorURL: ""},
		{FullName: "user/mirror", Fork: false, Archived: false, MirrorURL: "https://example.com/repo.git"},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			wantNames: []string{"user/fork"},
		},
		{
			name: "sources with archives",
			filter: &RepoFilter{
				IncludeSources:  true,
				IncludeForks:    false,
				IncludeArchived: true,
			...
			wantNames: []string{"user/source", "user/archived"},
		},
		{
			name: "forks and archives",
			filter: &RepoFilter{
			...
			wantNames: []string{"user/fork", "user/archived-fork"},
		},
		{
			name: "all types",
			filter: &RepoFilter{
			...
			wantNames: []string{"user/source", "user/fork", "user/archived", "user/archived-fork", "user/mirror"},
		},
		{
			name: "sources and mirrors",
			filter: &RepoFilter{
				IncludeSources:  true,
				IncludeForks:    false,
				IncludeArchived: false,
			...
			wantNames: []string{"user/source", "user/mirror"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got) != len(tt.wantNames) {
				t.Errorf("filterRepos() returned %d repos, want %d", len(got), len(tt.wantNames))
				return
			}
			for i, repo := range got {
				if repo.FullName != tt.wantNames[i] {
					t.Errorf("filterRepos()[%d] = %v, want %v", i, repo.FullName, tt.wantNames[i])
				}
			}
		})
	}
